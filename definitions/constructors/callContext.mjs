...
// ------------------------------------------------------------------------------------------------
import { isNotEmptyArray, beautify, dateEpochMsToIso } from '../../utils/jsUtils.mjs'
import { logD, logI, logT, logW, sysInfo, sysOnError } from '../../utils/logging.mjs'
  ...
  addError = (ctxMod, ctxFun, error) => {
    ...
    try {
      ...
      if (RudiError.isRudiError(error)) {
        ...
        if (ACTIVATE_LOG) logT(mod, fun, `this[DETAILS]: ${this[DETAILS]}`)
