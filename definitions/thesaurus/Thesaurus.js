...
const { isObject, isString, beautify } = require('../../utils/jsUtils')
...
module.exports = class Thesaurus {
  ...
  constructor(code, initValues) {
    ...
    // log.d(mod, fun, `${code}: ${beautify(initValues)}`)
  ...
  getLabels(lang) {
    ...
    try {
      ...
      log.d(mod, fun, beautify(this.#currentValues))
      const langLabels = {}
  ...
  #storeEnum = async (thesaurusValues) => {
    ...
    try {
      ...
      if (!this.#hasLabels) {
        ...
        // log.d(mod, fun, beautify(storableEntries))
        dbAction = await DynamicEnum.findOneAndUpdate(
          { [ENUM_CODE]: this.#code },
      ...
      // log.d(mod, fun, beautify(dbAction))
      return dbAction
    } catch (err) {
      log.w(mod, fun, err)
      throw RudiError.treatError(mod, fun, err)
